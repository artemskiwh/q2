  ...
  return (
    <div className="flex items-center gap-6 px-5 py-5">
      <div className="shrink-0 text-center">
        ...
        <div className="mt-1.5 text-xs text-black/45">{RESTAURANT.ratingCount} оценок</div>
      </div>
      <div className="flex-1 space-y-1.5">
        {DIST.map((pct, idx) => (
          <div key={idx} className="flex items-center gap-2">
            <span className="w-3 text-right text-[11px] text-black/45">{5 - idx}</span>
...
function Chips() {
  return (
    <div className="flex gap-2 overflow-x-auto px-5 pb-4 scrollbar-hide">
      {CHIPS.map((c) => (
        <span key={c} className="whitespace-nowrap rounded-full bg-black/[0.07] px-3.5 py-1.5 text-[13px] text-black/75">
          {c}
        </span>
      ))}
    </div>
...
function SortRow() {
  return (
    <div className="flex items-center justify-between px-5 py-3">
      ...
      <span className="inline-flex items-center gap-1 text-[13px] text-black/45">
        По новизне
    ...
    <div className="border-t border-black/10 px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-black/40">{r.count ?? "1 отзыв"}</p>
        </div>
        <span className="text-black/30">···</span>
      </div>
      <div className="mt-3 flex items-center gap-3">
        <Stars n={r.rating} size={15} />
        <span className="text-[13px] text-black/40">{r.date}</span>
      </div>
      <p className="mt-2.5 text-[14px] leading-relaxed text-black/80">{r.text}</p>
      <button
        onClick={() => setLiked((v) => !v)}
        className="mt-3 inline-flex items-center gap-2 rounded-md border border-black/15 px-3 py-1.5 text-[13px] text-black/55 transition-colors hover:bg-black/5"
      >
  ...
  return (
    <section id="reviews" className="relative py-24 md:py-32">
      <div className="container-page">
        ...
        <Reveal delay={100}>
          <div className="mx-auto mt-12 max-w-xl overflow-hidden rounded-2xl border border-black/12 shadow-[0_24px_60px_-30px_rgba(0,0,0,0.3)]">
            <GisPanel />
            <button
          ...
          >
            <div className="flex shrink-0 items-center justify-between border-b border-black/10 px-5 py-4">
              ...
              <button
                aria-label="Закрыть"
                onClick={onClose}
                className="grid h-9 w-9 place-items-center rounded-full text-black/70 transition-colors hover:bg-black/8"
              >
