...
// Товары для которых нужно найти изображения
const products = [
  { name: 'Rincoe Jelly 1500', category: 'одноразовые электронные сигареты' },
  { name: 'Smoant Charon Baby', category: 'pod системы' },
  { name: 'Vaporesso XROS 4', category: 'pod системы' },
  { name: 'Lost Vape Orion Bar', category: 'одноразовые электронные сигареты' },
  { name: 'GeekVape Wenax H1', category: 'pod системы' },
  { name: 'Elf Bar 600', category: 'одноразовые электронные сигареты' },
  { name: 'OXVA Xlim Pro 2', category: 'pod системы' },
  { name: 'HQD Cuvie Plus', category: 'одноразовые электронные сигареты' },
];
...
async function main() {
  ...
  for (const product of products) {
    ...
    if (imageUrl) {
      ...
      if (cloudinaryUrl) {
        ...
        console.log(`⚠️ Upload failed for ${product.name}`);
      }
    } else {
  ...
  // Обновляем lib/product-images.ts
